...
process_event_data <- function(event_name, K_values) {
  ...
  file_path <- paste0('ETT/PPI/Processed_files/PPI_bootstrap_DS_24_', event_name, '.ft')
...
create_plot <- function(event_name, K, graph_MC, HU_time) {
  ...
  plot.plr <- ggplot(graph_MC, 
                     aes(x = time_0, y = risk)) + 
    geom_line(aes(y = risk1, color = "PPI"), linewidth = 1.5) + 
    geom_line(aes(y = risk0, color = "Non-PPI"), linewidth = 1.5) +
    xlab("Months") + 
  ...
  ggsave(paste0("ETT/PPI/BT_Output/CI_K", K, "_HU", HU_time, "_", event_name, '_DF', ".png"), plot = plot.plr, width = 8, height = 8)
